...
SuggestedAction GameAI::suggestAction(const ShipState& ship_state) {
    ...
    //Highlight yellow box for debug
    ...
    //If list is empty, return an action of choice
    if(asteroid_observer.asteroids().begin() == nullptr)
         return SimpleActions::START_YAWING_CW;
        //return SuggestedAction{ SuggestedAction::YawingAntiClockwise, SuggestedAction::FiringTry};
    
    AsteroidListItem *targetAsteroid = nullptr;
    AsteroidList asteroidlist = asteroidsObserver().asteroids();
    targetAsteroid = TargetAsteroid((AsteroidList*) &asteroidlist, ship_state);

    if (targetAsteroid != nullptr) {
        
        //Calling the GetAngle function to shoot at the angle
        double theta = GetAngle(targetAsteroid);

        //Adjust firing angle
        if (ship_state.millidegree_rotation < theta)
            return SuggestedAction{ SuggestedAction::YawingClockwise, SuggestedAction::FiringTry};
        else if (ship_state.millidegree_rotation > theta)
            return SuggestedAction{ SuggestedAction::YawingAntiClockwise, SuggestedAction::FiringTry};
        else
            return SimpleActions::STOP_YAWING_AND_FIRE;
    }

    return SimpleActions::NO_CHANGE; //Absolutely nothing to do
